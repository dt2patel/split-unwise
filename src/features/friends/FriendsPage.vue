    ...
    <ion-content :fullscreen="true">
      <main class="friends-page">
        ...
        <form v-if="showingCreate" class="friend-form" @submit.prevent="createFriend">
          <ion-input v-model="displayName" class="friend-field" :label="t('friends.name')" label-placement="stacked" autocomplete="name" autocapitalize="words" :maxlength="120" :placeholder="t('friends.namePlaceholder')" />
          <ion-input v-model="email" class="friend-field" type="email" inputmode="email" autocomplete="email" :label="t('auth.email')" label-placement="stacked" :placeholder="t('friends.emailPlaceholder')" />
          <label class="friend-field friend-field--select"><span>{{ t('groups.currency') }}</span><select v-model="currency"><option v-for="code in SUPPORTED_CURRENCIES" :key="code" :value="code">{{ code }}</option></select></label>
