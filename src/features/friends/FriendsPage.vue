...
import { IonAvatar, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonInput, IonItem, IonLabel, IonList, IonNote, IonPage, IonSkeletonText, IonTitle, IonToolbar, onIonViewWillEnter } from '@ionic/vue'
import { add, chevronDown, chevronUp, copyOutline, peopleOutline } from 'ionicons/icons'
...
const manualInvitationUrl = ref<string>()
...
async function createFriend(): Promise<void> {
  ...
  try {
    ...
    creating.value = true; createError.value = undefined; notice.value = undefined; manualInvitationUrl.value = undefined
...
async function shareInvitation(): Promise<void> {
  ...
  manualInvitationUrl.value = result.status === 'manual' ? result.url : undefined
}
</script>
    ...
    <ion-content :fullscreen="true">
      <main class="friends-page">
        ...
        <form v-if="showingCreate" class="friend-form" @submit.prevent="createFriend">
          <ion-input v-model="displayName" class="friend-field" label-placement="stacked" autocomplete="name" autocapitalize="words" :maxlength="120" :placeholder="t('friends.namePlaceholder')">
            <span slot="label" class="friend-field__label">{{ t('friends.name') }}</span>
          </ion-input>
          <ion-input v-model="email" class="friend-field" type="email" inputmode="email" autocomplete="email" label-placement="stacked" :placeholder="t('friends.emailPlaceholder')">
            <span slot="label" class="friend-field__label">{{ t('auth.email') }}</span>
          </ion-input>
          <label class="friend-field friend-field--select"><span class="friend-field__label">{{ t('groups.currency') }}</span><select v-model="currency"><option v-for="code in SUPPORTED_CURRENCIES" :key="code" :value="code">{{ code }}</option></select></label>
        ...
        <p v-if="noticeCopy" id="friend-invitation-status" class="friends-page__notice" role="status" aria-live="polite">{{ noticeCopy }}</p>
        <section v-if="invitation" class="invitation-ready" aria-labelledby="friend-invitation-title">
          ...
          <textarea
            v-if="manualInvitationUrl"
            class="invitation-ready__manual-url"
            readonly
            :value="manualInvitationUrl"
            :aria-label="t('invite.urlAria')"
            aria-describedby="friend-invitation-status"
        ...
        <section v-else aria-labelledby="friend-list-title">
          ...
          <ion-list v-else class="friend-list" lines="none">
            <div v-for="friend in friends" :key="friend.id" class="friend-entry">
              ...
              <ion-list v-if="expandedFriendId === friend.id" class="friend-breakdown" lines="none" :data-breakdown-for="friend.id">
                <ion-item v-for="item in friend.breakdowns" :key="`${item.contextId}:${item.currency}`" class="friend-breakdown__link" :router-link="`/tabs/groups/${encodeURIComponent(item.contextId)}`" detail>
                  <ion-label class="friend-breakdown__copy"><strong>{{ item.contextName }}</strong><small>{{ item.contextKind === 'friendship' ? (friend.pending ? t('home.invitationPending') : t('friends.directExpenses')) : t('friends.sharedGroup') }}</small></ion-label>
                  <MoneyAmount slot="end" :money="absolute(item)" :direction="direction(item)" :label="directionLabel(item)" />
                </ion-item>
                <ion-item v-for="item in friend.unavailableContexts" :key="`unavailable:${item.contextId}`" class="friend-breakdown__link" :router-link="`/tabs/groups/${encodeURIComponent(item.contextId)}`" detail>
                  <ion-label class="friend-breakdown__copy"><strong>{{ item.contextName }}</strong><small>{{ friend.pending ? t('home.invitationPending') : t('friends.directExpenses') }}</small></ion-label>
                  <small slot="end" class="friend-breakdown__unavailable">{{ balancesLoading ? t('home.updatingBalances') : t('home.balanceUnavailable') }}</small>
                </ion-item>
              </ion-list>
            </div>
          </ion-list>
...
@media(max-width:360px){.friends-page{padding-inline:14px}.friend-row__end{min-width:72px;gap:5px}.friend-breakdown{margin-left:0}.friend-breakdown__link{--detail-icon-font-size:.875em}}
