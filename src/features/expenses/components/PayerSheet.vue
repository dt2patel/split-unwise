...
<template>
  <section ref="sheet" class="expense-sheet" data-sheet-scroll aria-labelledby="payer-title">
    <header class="expense-sheet__header"><button type="button" @click="emit('cancel')">Cancel</button><h2 id="payer-title">{{ reimbursement ? 'Refund received by' : 'Paid by' }}</h2><button type="button" data-action="apply-payers" @click="apply">Apply</button></header>
    ...
    <div class="sheet-list">
      <label v-for="member in members" :key="member.id">
        ...
        <input v-if="selected(member.id)" inputmode="decimal" :value="amount(member.id)" :data-payer-id="member.id" :aria-label="`${member.displayName} ${reimbursement ? 'received' : 'paid'} amount`" :aria-invalid="errorKind === 'amount' && (!errorParticipantId || errorParticipantId === member.id) ? 'true' : undefined" :aria-describedby="errorKind === 'amount' && (!errorParticipantId || errorParticipantId === member.id) ? 'payer-error' : undefined" @input="update(member.id, $event)">
      </label>
    </div>
