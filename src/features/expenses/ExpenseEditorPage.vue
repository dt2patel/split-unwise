...
<template>
  <ion-page :ref="setPresentingElement" class="expense-editor-page">
    ...
    <ion-modal :is-open="Boolean(store.activeSheet)" :presenting-element="presentingElement" :can-dismiss="modalCanDismiss" @input="markSheetDirty" @change="markSheetDirty" @did-dismiss="closeSheet">
      <context-sheet v-if="store.activeSheet === 'context'" class="ion-content-scroll-host" :model-value="store.editor.groupId" :groups="store.availableGroups" @apply="applyContext" @cancel="closeSheet" />
      <payer-sheet v-else-if="store.activeSheet === 'payers'" class="ion-content-scroll-host" :model-value="store.editor.payments" :members="store.members" :currency="store.editor.currency" :total-minor-amount="totalMinorAmount" @apply="applyPayers" @cancel="closeSheet" />
      <participant-sheet v-else-if="store.activeSheet === 'participants'" class="ion-content-scroll-host" :model-value="store.editor.participants" :members="store.members" @apply="applyParticipants" @cancel="closeSheet" />
      <split-editor v-else-if="store.activeSheet === 'split'" class="ion-content-scroll-host" :model-value="store.editor.split" :participants="store.members.filter((member) => store.editor.participants.includes(member.id))" :currency="store.editor.currency" :total-minor-amount="totalMinorAmount" @apply="applySplit" @cancel="closeSheet" @dirty="markSheetDirty" />
      <recurrence-sheet v-else-if="store.activeSheet === 'recurrence'" class="ion-content-scroll-host" :model-value="store.editor.recurrence" :occurrence-edit-scope="store.editor.occurrenceEditScope" :is-recurring-instance="Boolean(store.recurringTemplateId)" :date="store.editor.date" @apply="applyRecurrence" @cancel="closeSheet" @dirty="markSheetDirty" />
      <receipt-review v-else-if="store.activeSheet === 'receipt'" class="ion-content-scroll-host" :model-value="receiptItems" :members="store.members.filter((member) => store.editor.participants.includes(member.id))" :currency="store.editor.currency" :total-minor-amount="totalMinorAmount" :provider-message="store.receiptMessage" :image-url="receiptPreviewUrl" :durability="store.receiptDurability" @confirm="confirmReceipt" @cancel="closeSheet" @dirty="markSheetDirty" />
    </ion-modal>
  </ion-page>
</template>
