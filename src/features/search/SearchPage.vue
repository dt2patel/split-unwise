...
import { IonAccordion, IonAccordionGroup, IonBackButton, IonButtons, IonContent, IonHeader, IonItem, IonLabel, IonList, IonPage, IonSearchbar, IonTitle, IonToolbar } from '@ionic/vue'
...
import type { ExpenseSearchFilters, ExpenseSearchResult } from '../../domain/search'
import { loadExpenseSearchSource, searchLoadedExpenses, type ExpenseSearchSource } from '../premium/premiumData'

const route = useRoute()
// Ionic keeps this page alive under the pages it opens, so it reads its own route once instead of following later navigation.
const isGroupSearch = route.name === 'group-search'
const groupId = typeof route.params.groupId === 'string' && isStrictId(route.params.groupId) ? route.params.groupId : undefined
const backPath = isGroupSearch ? groupId ? `/tabs/groups/${encodeURIComponent(groupId)}` : '/tabs/groups' : '/tabs/home'

const query = ref(''); const selectedGroups = ref<string[]>([]); const selectedParticipants = ref<string[]>([]); const selectedCategories = ref<string[]>([]); const currency = ref(''); const dateFrom = ref(''); const dateTo = ref(''); const minimum = ref(''); const maximum = ref('')
const source = shallowRef<ExpenseSearchSource>(); const loading = ref(false); const loadError = ref('')
const searchbar = ref<ComponentPublicInstance>()

// Results follow every keystroke and filter change: the expenses load once, and searching them is local and instant.
const outcome = computed<{ readonly result?: ExpenseSearchResult; readonly error?: string }>(() => {
  if (!source.value) return {}
  try { return { result: searchLoadedExpenses(source.value, currentFilters()) } } catch (reason) { return { error: reason instanceof Error ? reason.message : String(reason) } }
})
const result = computed(() => outcome.value.result)
const error = computed(() => loadError.value || outcome.value.error || '')
// The pickers offer everything that can be chosen, whatever is selected or typed now.
const facets = computed(() => source.value ? searchLoadedExpenses(source.value, {}).facets : undefined)
const groups = computed(() => facets.value?.groups ?? [])
const participants = computed(() => facets.value?.participants ?? [])
const categories = computed(() => facets.value?.categories ?? [])
const currencies = computed(() => facets.value?.currencies ?? [])
const resultLabel = computed(() => `${result.value?.items.length ?? 0} ${(result.value?.items.length ?? 0) === 1 ? 'result' : 'results'}`)
onMounted(() => { void labelSearchInput(); void load() })
...
async function load(): Promise<void> {
  ...
  try {
    if (isGroupSearch && !groupId) throw new Error('Open search from a valid group link.')
    source.value = await loadExpenseSearchSource(groupId)
  } catch (reason) { loadError.value = reason instanceof Error ? reason.message : String(reason) } finally { loading.value = false }
}
function currentFilters(): ExpenseSearchFilters {
...
/** The keyboard's Search key only puts the keyboard away: the results are already current. */
function dismissKeyboard(): void { if (document.activeElement instanceof HTMLElement) document.activeElement.blur() }
...
function destination(group: string, expense: string): string { const origin = isGroupSearch ? 'groups' : 'home'; return `/tabs/${origin}/expenses/${encodeURIComponent(expense)}?groupId=${encodeURIComponent(group)}` }
</script>
...
<template>
  <ion-page><ion-header translucent><ion-toolbar><ion-buttons slot="start"><ion-back-button :default-href="backPath" text="Back" /></ion-buttons><ion-title>Search</ion-title></ion-toolbar></ion-header>
    <ion-content :fullscreen="true"><main class="search-main"><p class="eyebrow">{{ isGroupSearch ? 'This group' : 'All groups' }}</p><h1>Search</h1><p class="intro">Match words plus exact groups, people, categories, dates, amounts, and currency.</p>
      <form class="search-form" @submit.prevent="dismissKeyboard">
        <div class="search-query"><label id="search-query-label" for="search-query">Description or notes</label><ion-searchbar ref="searchbar" v-model="query" class="search-query__field" type="search" enterkeyhint="search" autocomplete="off" placeholder="Coffee, cabin, train…" @keydown.enter.prevent="dismissKeyboard" /></div>
      ...
      <p v-if="loading" role="status" class="coverage">Loading expenses…</p>
      ...
      <section v-if="result" aria-labelledby="results-heading"><header class="results-heading"><h2 id="results-heading">Results</h2><span data-testid="result-count">{{ resultLabel }}</span></header><p data-testid="coverage" class="coverage">{{ coverage() }}</p>
        ...
        <ion-list v-else class="result-list" lines="full">
          <ion-item v-for="item in result.items" :key="`${item.group.id}:${item.expense.id}`" :data-expense-id="item.expense.id" :router-link="destination(item.group.id, item.expense.id)" detail>
            <ion-label><strong>{{ item.expense.description }}</strong><small>{{ item.group.name }} · {{ item.expense.category }} · <time class="result-date" :datetime="item.expense.date">{{ item.expense.date }}</time></small></ion-label><b slot="end">{{ formatMoney(item.expense.total) }}</b>
          </ion-item>
        </ion-list>
      </section>
