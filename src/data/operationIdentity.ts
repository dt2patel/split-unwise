...
/** Exact SHA-256 fallback for plain-HTTP previews where Web Crypto is unavailable. */
function fallbackSha256(input: Uint8Array): Uint8Array {
  const padding = (64 - ((input.length + 9) % 64)) % 64
  const message = new Uint8Array(input.length + 9 + padding)
  message.set(input)
  message[input.length] = 0x80
  const messageView = new DataView(message.buffer)
  const bitLength = input.length * 8
  messageView.setUint32(message.length - 8, Math.floor(bitLength / 0x100000000))
  messageView.setUint32(message.length - 4, bitLength >>> 0)
