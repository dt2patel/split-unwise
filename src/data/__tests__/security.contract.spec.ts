...
describe('Firebase security contract', () => {
  it('authorizes groups from active membership documents and default-denies unknown paths', () => {
    ...
    expect(firestoreRules).toMatch(/match \/groups\/\{groupId\}[\s\S]*?allow update: if activeAccount\(\)[\s\S]*?validGroupJoin\(groupId\)/)
  ...
  it('keeps operation ledgers server-only while narrowly allowing audited Spark mutations', () => {
    ...
    expect(firestoreRules).toMatch(/match \/revisions\/\{revisionId\}[\s\S]*?allow create: if activeAccount\(\)[\s\S]*?validSparkExpenseVersionCreate\(groupId, expenseId, revisionId, request\.resource\.data\)[\s\S]*?allow update: if validAccountDeletionRevision\(groupId, request\.resource\.data\);[\s\S]*?allow delete: if false;/)
    ...
    expect(firestoreRules).toContain('allow create: if activeAccount() && validSparkCommentCreate(groupId, commentId, request.resource.data);')
    expect(firestoreRules).toContain('allow update: if activeAccount() && validSparkCommentDelete(groupId, commentId, request.resource.data);')
    ...
    expect(firestoreRules).toMatch(/allow update: if activeAccount\(\) && settingId == 'defaults'[\s\S]*?request\.resource\.data\.lastCommandKind == 'group\.currency-conversion'/)
    expect(firestoreRules).toContain('? validSparkCurrencySettingsDocumentUpdate(groupId, request.resource.data)')
    expect(firestoreRules).toContain(': validSparkSettingsUpdate(groupId, request.resource.data));')
    expect(firestoreRules).toContain('validSparkCurrencyConversionCreate(groupId, conversionId, request.resource.data)')
    expect(firestoreRules).toContain('validSparkCurrencyRateCreate(groupId, conversionId, sourceCurrency, request.resource.data)')
    expect(firestoreRules).toContain("allow update: if activeAccount() && snapshotId == 'current' && validSparkBalanceSettingsUpdate(groupId);")
    expect(firestoreRules).toMatch(/match \/settlements\/\{settlementId\}[\s\S]*?allow create: if activeAccount\(\) && validSparkSettlementCreate[\s\S]*?allow update: if activeAccount\(\) && validSparkSettlementUpdate[\s\S]*?allow delete: if false;/)
    expect(firestoreRules).toContain('allow create: if owns(uid) && validProfile(request.resource.data)')
    expect(firestoreRules).toMatch(/match \/notificationReads\/\{notificationId\}[\s\S]*?allow get: if owns\(uid\);[\s\S]*?allow list: if owns\(uid\) && bounded\(100\);[\s\S]*?allow create: if activeAccount\(\) && validSparkNotificationReadReceipt\(uid, notificationId, request\.resource\.data\);[\s\S]*?allow delete: if accountDeletionOwnsPrivateData\(uid\); allow update: if false;/)
    ...
    expect(firestoreRules).toMatch(/match \/recurringTemplates\/\{templateId\}[\s\S]*?allow update: if activeAccount\(\) && validSparkRecurringTemplateUpdate\(groupId, templateId, request\.resource\.data\);[\s\S]*?validAccountDeletionRecurringActor\(groupId, request\.resource\.data\);[\s\S]*?allow delete: if false;/)
    expect(firestoreRules).toContain('allow create: if activeAccount() && validNewGroup(groupId, request.resource.data) && ownerBundleExists(groupId)')
