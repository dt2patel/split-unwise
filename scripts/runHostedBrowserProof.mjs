...
async function verifyAuthenticatedMobileJourney() {
  ...
  try {
    ...
    if (pageErrorCount > 0 || consoleErrorCount > 0) throw new Error(`Hosted browser journey emitted ${pageErrorCount} page errors and ${consoleErrorCount} console errors.`)
    await context.close()
  } finally {
    await browser.close()
  }
