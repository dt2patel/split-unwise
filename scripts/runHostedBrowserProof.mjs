...
async function verifyReimbursementWorkflow(page) {
  ...
  const detailTitle = (await detail.locator('ion-title').textContent())?.trim()
  if (detailTitle !== 'Reimbursement') throw new Error(`Hosted reimbursement detail exposed an unexpected toolbar title: ${detailTitle ?? 'missing'}.`)
