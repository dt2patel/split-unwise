...
Task 9: complete from baseline `7783451`; unlocked currency-isolated totals and animated accessible charts, bounded authorized search with stable facets, versioned manager-owned shared defaults, auditable CSV/JSON exports, and explicit provider-gated import/FX contracts are integrated. Self-review closed invalid-route scope widening, cross-group CSV/revision leakage, stale-default edit blocking, settings-conflict refresh, and impossible-calendar timestamp acceptance. Final verification is 55 files/596 tests, typecheck, production build with 353 modules transformed, and diff check passing. Firebase search/export providers remain Task 11 and browser/device visual proof remains Task 12.

Task 10: complete from baseline `216652b`; session-independent Firebase boot/auth, fatal broken-config handling, one named bootstrap, email/Google account lifecycle, guarded one-shot returns, native iOS grouped Account settings, pre-paint appearance, complete ISO currency preferences, reversible sign-out quiescing, exact-principal local-data clearing, and fragment-safe invitation/share flows are integrated. Self-review closed optional-capability downgrade errors, missing-profile loading deadlock, live queue/receipt clearing, targeted-email absence, and field-linked credential validation. Final verification is 65 files/631 tests, typecheck, production build with 379 modules transformed, and diff check passing. Callable-backed production mutations/invitations remain Task 11 and browser/device visual proof remains Task 12.
