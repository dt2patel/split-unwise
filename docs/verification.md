...
Current hosted Add Expense, mobile-group, recurring-expense, and cross-context-move feature source: `d226693c8a3afae050c2274fd6ec6e6bd441adce`

The compatible Spark rules and feature Hosting bundle are deployed. `pnpm test:hosted` passed against the exact feature commit with disposable verified accounts, the real production Auth/Firestore services, and a real hosted Chrome journey at 390 x 844.
...
| --- | --- | --- |
| Unit and component suite | Pass | `pnpm test`: 89 files passed, 7 skipped; 897 tests passed, 72 skipped |
...
| Spark Auth/group/ledger flows | Pass | `pnpm test:firebase`: 10 emulator-backed flows, including collaborative expense mutation, replay-safe cross-context move, recurrence concurrency/replay, and occurrence/future/cancellation semantics |
| Functions and service behavior | Pass | `pnpm test:firebase`: 29 Functions/service tests; 64 total Firebase checks across rules, Spark flows, and Functions |
| Disposable hosted proof | Pass | `pnpm test:hosted`: the production SDK proof passed in 49.4 seconds and the subsequent hosted Chrome journey passed. The proof included real invitation acceptance, a replay-safe ordinary expense move with both immutable audit halves, and the 390 x 844 context-card interaction through production's default browser transport. |
...
| Firebase Hosting | Pass | Existing compatible Firestore rules remained unchanged and the production Hosting bundle was released for the feature source identified above; root, nested route, startup assets, build metadata, and the authenticated mobile browser journey passed. |
...
- Edit an ordinary expense and choose another group or friend in the Ionic context card. The source group remains the edit authority while the selected context becomes the destination.
- Spark commits the source tombstone plus immutable source revision and the destination expense in one Firestore transaction. Both the actor and every payer/split participant must be active in the destination, and the destination revalidates currency and exact minor-unit totals.
- The destination half has a distinct bounded replay identity, revision, and activity record. Replaying the same move converges on the same records; changing its payload under that identity is rejected.
- Pending moves hide the source row and show the destination row. A failed move restores the source and does not leak a destination copy. Recurring expenses must be stopped first, and existing remote receipts must be removed before moving and reattached in the destination.
- Unit, journal-projection, Spark emulator, Functions emulator, and production tests cover successful moves, replay, authorization, recurrence/receipt guards, balance changes, immutable revisions, account activity, and failed-state projection. Hosted Chrome completed the card-modal move at 390 x 844, found the fresh destination row without overflow, and confirmed the source row was absent.
...
- A tenth live production run exercised feature source `d226693c8a3afae050c2274fd6ec6e6bd441adce`. The 49.4-second SDK proof moved an ordinary two-user expense between real shared contexts, replayed it without duplication, retained the source tombstone plus both immutable audit halves, updated both balances, and exposed exactly two account-activity entries. Hosted Chrome then moved a browser-created expense through the Ionic context card at 390 x 844, found the fresh target row without overflow, confirmed the source row was absent, and completed the invitation and swipe-back journeys. The disposable fixture runner removed its bounded Auth/Firestore data.
...
- The production group detail, expanded More actions, lazy Activity, and edit-context card surfaces were inspected at 390 x 844 after the mobile fix. The route now has one valid Ionic page root; header/back/settings controls, the sticky Expenses/Activity switch, and the floating add-expense action remain usable. Four primary actions fit without clipping, secondary actions expand in a native tray, and expense rows preserve a readable description plus one compact financial column. The hosted move destination had no horizontal overflow, and the final navigation produced zero console errors or warnings. Evidence is under [mobile-audit](../output/playwright/mobile-audit/).
