...
| --- | --- | --- |
| Unit and component suite | Pass | `pnpm test`: 77 files passed, 7 skipped; 714 tests passed, 41 skipped |
...
- The latest proof removed 20 exact Firestore documents, including the immutable receipt and private Spark cursor, after the test deleted both Auth accounts. Follow-up Firebase MCP queries returned zero matching Auth users, proof groups, and invitations.

## Accessibility and interaction evidence

- The production group detail and activity surfaces were inspected at 390 x 844 after the mobile fix. The detail outlet occupies the full phone width, desktop-only group-list work is skipped, the header/back/settings controls and sticky Expenses/Activity switch remain usable, the floating add-expense action clears the journal, and long content scrolls without horizontal clipping. The notification release's hosted Auth shell was independently rechecked at the same viewport with zero console errors. Evidence is under [mobile-audit](../output/playwright/mobile-audit/).
