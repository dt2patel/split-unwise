...
| --- | --- | --- |
| Unit and component suite | Pass | `pnpm test`: 77 files passed, 7 skipped; 712 tests passed, 40 skipped |
| Type safety | Pass | `pnpm typecheck` |
| Firestore rules | Pass | `pnpm test:firebase`: 14 emulator-backed rules tests, including audited expense and comment/activity mutations plus manual settlement creation, replay, immutable activity, one authorized void, and denied outsider/invalid/physical-delete writes |
| Auth/group/invite/expense flow | Pass | Five emulator-backed Spark flows plus a live production run through the real Auth service: two temporary Email/Password accounts, sign-out/in, group creation, private invitation acceptance, profile/preferences, expense edit/replay, parallel group reads, manual settlement record/replay/void/replay, and a decoded two-member list |
| Functions and service behavior | Pass | `pnpm test:firebase`: 16 emulator-backed Functions tests; 35 total Firebase emulator checks |
...
- The production group detail and activity surfaces were inspected at 390 x 844 after the mobile fix. The detail outlet occupies the full phone width, desktop-only group-list work is skipped, the header/back/settings controls and sticky Expenses/Activity switch remain usable, the floating add-expense action clears the journal, and long content scrolls without horizontal clipping. Evidence is under [mobile-audit](verification-assets/mobile-audit/).
- Group navigation now exposes the header/hero/action shell as soon as the group document arrives, preserves an already loaded group during refresh, renders journal-shaped skeleton rows while the remaining Firebase reads complete, and coalesces duplicate same-group loads.
