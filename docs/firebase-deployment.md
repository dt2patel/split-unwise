...
Occurrence-only edits preserve the template. Any active group member may make a future-series edit from the exact current source/occurrence frontier or cancel at the exact template revision. These mutations never rewrite creator attribution or delete the source or posted occurrences; their revision and activity records identify the actual acting member. Rules revalidate all stored payers and split participants on every materialization; removing any involved participant stops future posting while past expenses remain.
