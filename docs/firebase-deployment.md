...
This is client catch-up, not unattended scheduling. A billed Firebase project and deployed Functions scheduler are required for due items to post while no authorized client opens the app. Before promoting this candidate, deploy the matching Firestore rules and Hosting bundle together, then run the disposable two-user hosted proof against the exact release. That proof is prepared in source but was not run during this no-deploy task.
...
| Firestore rules / indexes | Audited Spark expense, comment/activity, manual-settlement, private notification-read, and validated current-expense projection rules released successfully from source `3ba53c1e6b716ba8b01265e540b044d4d1ac7b3f`; existing indexes unchanged |
...
| Production URL / release | `https://split-unwise-aditya.web.app` and `https://split-unwise-aditya.firebaseapp.com`; Firebase load/mobile-group feature source `3ba53c1e6b716ba8b01265e540b044d4d1ac7b3f`; `/build-info.json` records the exact final release commit |
...
| Spark recurring expenses | Local release candidate only; not deployed in Task 5. Unit/rules/emulator coverage exercises atomic source/template creation, deterministic concurrent catch-up, removed-participant denial, occurrence and exact-frontier future edits, exact-revision cancellation, retained history, and the 24-item client cap. The production proof has been extended for two disposable users and bounded cleanup, but must be run after the matching rules and Hosting deployment. True unattended scheduling still requires billed Functions. |
